...
import zarr
#NumCodecs 是zarr用到的
#NumCodecs is available which provides a common interface to various compressor libraries including
# Blosc, Zstandard, LZ4, Zlib, BZ2 and LZMA.
...
# groups can be stored in memory, on disk, or via other storage systems that support a similar interface
...
class ReplayBuffer:
    ...
    def __init__(self, 
        ...
        # Union用于指定变量的多种可能类型。在这里，root的类型可以是zarr.Group，
        # 也可以是Dict[str, dict]):
        """
        ...
        ##dict.items() 方法返回一个包含字典所有键值对的可迭代对象。
        # 每个键值对以元组的形式返回，键值对的顺序是不确定的
        for key, value in root['data'].items():
            ##value.shape[0] 表示什么？episode还是time_step？   应该是获取了episode的个数
            # 这里的episode_ends应该是指episode的结束时间（待确定）
    ...
    def copy_from_store(cls, src_store, store=None, keys=None, 
        ...
        Load to memory.
        """
        # zarr.group函数的作用是从现有存储后端对象中获取所有现有的数据和元数据，并创建一个zarr.Group对象实例来表示该存储
        # src_root将包含与存储后端对象关联的所有数据和元数据，您将能够使用src_root来对其进行进一步操作，例如读取、写入、创建子组等
    ...
    # 类方法是定义在类上的方法，而不是定义在类实例上的方法，因此可以在不创建类实例的情况下调用
    # 类方法的第一个参数通常被命名为 cls，它代表该类本身，而非类的实例。通过这个参数，可以访问类的属性和方法
    @classmethod
    ...
    #n_steps是最后一个episode结束时间，即最后一个episode的最后一个time_step的索引。

    ####说明episode_ends的行数是episode的个数。
    @property
    def n_episodes(self):

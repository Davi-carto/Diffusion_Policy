...
class LowdimMaskGenerator(ModuleAttrMixin):
    def __init__(self,
        ...
        # obs mask setup
        max_n_obs_steps=2,
        ##设置obs为固定步数还是随机步数，true则为max_n_obs_steps，false则为1~max_n_obs_steps之间的随机数
        fix_obs_steps=True,
        # action mask
    ...
    def forward(self, shape, seed=None):
        ...
        #torch.Generator(device=device) 用于创建指定设备上的随机数生成器。（random number generator）
        ...
        ##为batch中每个样本生成steps=1～T，
        steps = torch.arange(0, T, device=device).reshape(1,T).expand(B,T)
        ##steps.shape=[B,T]
        ##obs_steps.shape=[B,T_o]
        obs_mask = (steps.T < obs_steps).T.reshape(B,T,1).expand(B,T,D)
        obs_mask = obs_mask & is_obs_dim

        # generate action mask
        if self.action_visible:
            ###action_steps = max(obs_steps - 1, 0)

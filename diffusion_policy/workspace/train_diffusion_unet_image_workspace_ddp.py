...
class TrainDiffusionUnetImageWorkspaceDDP(BaseWorkspace):
    ...
    def run(self):
        ...
        train_dataloader = DataLoader(dataset, sampler=DistributedSampler(dataset), **cfg.dataloader)
        ...
        # device transfer
        device = torch.device(cfg.rank)

        # 构建 DDP 模型
        self.model = DDP(self.model, device_ids=[cfg.rank])
        if self.ema_model is not None:
            self.ema_model = DDP(self.ema_model, device_ids=[cfg.rank])
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                # 在每个 epoch 开始时对 DistributedSampler 调用 set_epoch() 方法对于在多个 epoch 中正确执行改组非常必要。
                # 否则，每个 epoch 中都将使用相同的排序。
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        # device transfer
                        batch = dict_apply(batch, lambda x: x.to(cfg.rank, non_blocking=True))
                ...
                # checkpoint
                if (self.epoch % cfg.training.checkpoint_every)  == 0 and cfg.rank == 0:
                    # checkpointing
